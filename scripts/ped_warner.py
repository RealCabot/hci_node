...
class Pedestrian_Warner:
    ...
    def warn_corner(self, msg):
        if self.cornerPts is not None and len(self.cornerPts) > 0:
            ...
            curr_pt = np.array([[coord.x, coord.y, coord.z]])
            dist = distance.cdist(self.cornerPts, curr_pt, 'euclidean')
            #print(self.cornerPts, curr_pt)
            min_dist = np.amin(dist)
            arg_min = np.argmin(dist)
            print('ind: ',arg_min, 'dist:',min_dist)
